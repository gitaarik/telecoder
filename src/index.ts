...
async function autoResumeAfterReload(bot: Bot, notified: Set<string>): Promise<boolean> {
 ...
 */
async function autoContinueOnStartup(bot: Bot, notified: Set<string>): Promise<void> {
  const activeSessions = sessionHistory.getAllActiveSessions();
  const allowedIds = new Set([
    ...config.ALLOWED_USER_IDS,
    ...config.ALLOWED_GROUP_IDS,
 ...
 */
async function notifyInterruptedSessions(bot: Bot, notified: Set<string>): Promise<void> {
 ...
 *
 * Only the session-driven paths above speak on startup, and each has its own
 ...
 */
async function notifyRestartComplete(bot: Bot, notified: Set<string>): Promise<void> {
  const allowedIds = new Set([
    ...config.ALLOWED_USER_IDS,
    ...config.ALLOWED_GROUP_IDS,
  ]);
  ...
  try {
    ...
    await bot.api.sendMessage(chatId, `✅ ${config.BOT_NAME} restarted and is ready.`, threadOpts);
