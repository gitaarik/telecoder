import { processMessageForTelegram } from './src/telegram/markdown.js';
...
console.log('=== Testing message processing and chunking ===');

try {
  const chunks = processMessageForTelegram(testText, 4096);
  console.log(`Split into ${chunks.length} chunks:`);

  chunks.forEach((chunk, index) => {
    console.log(`\n--- Chunk ${index + 1} (${chunk.length} chars) ---`);
    console.log(`First 100 chars: ${chunk.substring(0, 100).replace(/\n/g, '\\n')}`);
    console.log(`Last 100 chars: ${chunk.substring(chunk.length - 100).replace(/\n/g, '\\n')}`);

    // Check for problematic patterns that might cause MarkdownV2 to fail
    ...
    problemPatterns.forEach((pattern, i) => {
      if (pattern.test(chunk)) {
        console.log(`  ⚠️ Pattern ${i + 1} matched - potential markdown issue`);
      }
    });
  });
